...
process input = concatMap solve problem
  where problem = init $ splitOn ["0"] $ lines input
        ...
        solve x = head x ++ "\n" ++ build
          where build = concatMap writeCommand $ intervalNotation $ compress $ mconcat $ map readCommand (tail x)

readCommand :: String -> StepFunction Int Code
readCommand s = toStepFunction ([read a, read b + 1], [Identity, Code (unwords [c,d]), Identity])
